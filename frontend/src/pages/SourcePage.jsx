...
import {
  Box, Text, Spinner, Center, Heading, Table, Thead,
  Tbody, Tr, Th, Td, useColorModeValue, useDisclosure, Button,
  Select, Modal, ModalOverlay, ModalContent, ModalHeader, 
  ModalFooter, ModalBody, SimpleGrid
...
function SourcePage({ onLogout }) {
  ...
  const fetchStats = async (fromDate = null, toDate = null) => {
    ...
    try {
      ...
      if (res.status === 200 && res.data.status === 'ok') {
        ...
        throw new Error(res.data.message || 'Ошибка загрузки статистики')
      }
    } catch (err) {
  ...
  const fetchSourceName = async () => {
    try {
      ...
      if (res.status === 200 && res.data.status === 'ok') {
        ...
        throw new Error(res.data.message || 'Ошибка при загрузке источника')
      }
    } catch (err) {
      console.error('Ошибка при загрузке источника:', err)
      setSourceName(`Источник #${id}`)
    }
  }
  ...
  const isApplyDisabled = !selectedStartDate || !selectedEndDate || new Date(selectedStartDate) > new Date(selectedEndDate)
  ...
  return (
    <Box>
      ...
      <Box px={5} py={8}>
        <Heading size="lg" mb={4} color="purple.600"><InfoIcon boxSize={7} /> Статистика по источнику "{sourceName}"</Heading>
        ...
        <ModalContent>
          ...
          <ModalFooter>
            <Button
              ...
              onClick={() => {
                fetchStats(selectedStartDate, selectedEndDate)
                closeWeekModal()
              }}
            >
              Применить
